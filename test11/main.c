...
{
    if(fcntl(fd,F_SETLK,lock) == 0) //成功设置
    {
...
/* 测试锁,当发现参数lock能别锁定时返回零. */
int lock_test(int fd, struct flock* lock)
{
    if(fcntl(fd,F_GETLK,lock) == 0) //执行成功
    {
        if(lock->l_type == F_UNLCK) //测试时发现可以按参数设置锁
        {
...
int main()
{
    ...
    /* 打开或创建文件 */
    if((fd = open("text1",O_CREAT|O_TRUNC|O_RDWR,S_IRWXU)) == -1) {
        my_err("open",__LINE__);
    }
    if(write(fd,"text lock",10) != 10) {
